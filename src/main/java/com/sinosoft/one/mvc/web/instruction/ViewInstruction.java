 ...
 */
package com.sinosoft.one.mvc.web.instruction;
...
import com.sinosoft.one.mvc.web.impl.view.ViewDispatcher;
import com.sinosoft.one.mvc.web.impl.view.ViewDispatcherImpl;
import com.sinosoft.one.mvc.web.impl.view.ViewPathCache;
...
public class ViewInstruction extends AbstractInstruction {
    ...
    private String getViewPath(InvocationBean inv, String viewName) throws IOException {
        ...
        if (viewName.charAt(0) == '/'
                && !viewName.startsWith(MvcConstants.VIEWS_PATH_WITH_END_SEP)) {
            return viewName;
        }
        // 其他的按惯例行走
        String viewRelativePath;
        if (viewName.startsWith(MvcConstants.VIEWS_PATH_WITH_END_SEP)) {
            viewRelativePath = "";
            viewName = viewName.substring(MvcConstants.VIEWS_PATH_WITH_END_SEP.length());
        } else {
